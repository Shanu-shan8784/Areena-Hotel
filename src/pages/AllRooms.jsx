...
const RadioButtons = ({label, selected = false, onChange = ()=>{}})=>{
    return (
      <label className='flex gap-3 items-center cursor-pointer mt-2 text-sm'>
...
function AllRooms() {
  ...
  return (
    <div className='flex flex-col-reverse lg:flex-row items-start justify-between
     pt-28 md:pt-35 px-4 md:px-16 lg:px-24 xl:px-32 mt-10'>
      <div>
        <div className='flex flex-col items-start text-left'>
            <h1 className='font-playfair text-xl md:text-[40px]'>Hotel Rooms</h1>
        ...
        {roomsDummyData.map((room)=>(
          // Fixed: Added unique key and added flex for side-by-side layout
            <div key={room._id} className='flex flex-col md:flex-row gap-6 mb-10 items-start py-10 border-b border-gray-300 last:pb-30 last:border-0'>
                    ...
                    <div className='flex flex-wrap items-center mt-3 mb-6 gap-4'>
                        {room.amenities.map((item,index)=>(
                          <div key={item} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-[#F5F5ff]/70">
                            <img src={facilityIcons[item]} alt={item} className="w- h-5" />
      ...
      {/* Filters */}
      <div className='bg-white border w-70 md:w-80 border-gray-300 text-gray-600 max-lg:mb-8 lg:mt-16'>
        ...
        <div className={`${openFilters ? "h-auto": "h-0 lg:h-auto"} overflow-hidden transition-all duration-700`}>
          ...
          <div className='px-5 pt-5 pb-7'>
            ...
            {sortOptions.map((option,index)=>(
                <RadioButtons key={index} label={option}/>
            ))}
          </div>
        </div>
      </div>
